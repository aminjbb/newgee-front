<template>
    <div class="match-card-win  mt-10">
        <v-row justify="space-between" class="pa-11">
            <div>
                <v-row>
                    <div class="game-avatar-box border-g">
                        <img src="~/assets/img/5.png" height="106" width="106" alt="">
                    </div>
                    <div class="mt-6 mx-3">
                        <div>
                            <span class="white--text">Luca burn</span>
                        </div>
                        <div class="">
                            <span class="success--text t40500">26</span>
                        </div>
                    </div>
                </v-row>
            </div>
            <div class="position__relative">
                <div class="win-box text-center">
                    <span class="t35500 white--text">
                        Victory
                    </span>
                </div>
                <div class="time-box text-center border-g pt-3">
                    <div class="t40500 white--text vs-box">VS</div>
                    <span class="t14400 white--text">
                        Apr 28, 2022 8:00 PM
                    </span>
                </div>

                <div class="text-center pt-3">

                    <span class="t14400 white--text">
                        Round of 16
                    </span>
                </div>

            </div>
            <div>
                <v-row>

                    <div class="mt-6 mx-3">
                        <div>
                            <span class="white--text">Luca burn</span>
                        </div>
                        <div class="">
                            <span class="RedOrange--text t40500">25</span>
                        </div>
                    </div>
                    <div class="game-avatar-box border-w">
                        <img src="~/assets/img/5.png" height="106" width="106" alt="">
                    </div>
                </v-row>
            </div>
        </v-row>
    </div>
</template>

<script>
export default {

}
</script>